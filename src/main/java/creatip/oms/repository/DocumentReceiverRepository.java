...
public interface DocumentReceiverRepository extends JpaRepository<DocumentReceiver, Long> {
    ...
    @Query(
        value = "select dc from DocumentReceiver dc where dc.header.deliveryStatus=1 and dc.receiver.id=?1 and dc.status=0 and dc.delFlag='N'"
    )
    ...
    List<DocumentReceiver> findByHeaderIdAndReceiverIdAndReceiverType(Long headerId, Long recieverId, short receiverType);
}
